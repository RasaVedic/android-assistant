 ...
 * API key is read from SharedPreferences (set via Settings) or BuildConfig.
 */
class GeminiHelper(private val context: Context) {
    ...
    suspend fun interpretCommand(userInput: String): String? = withContext(Dispatchers.IO) {
        ...
        val prompt = """
            You are an Android voice assistant command interpreter called Aria.
            
            The user said: "$userInput"
            
            Classify this into exactly one of these actions. Respond with ONLY the action line, nothing else:
            - OPEN <app_name>
            - CALL <name_or_number>
            - ALARM <time>   (e.g. ALARM 7:00 am)
            - SELFIE front   (take front camera selfie)
            - SELFIE back    (take back camera photo)
            - SEARCH <query>
            - NAVIGATE back
            - NAVIGATE home
            - NAVIGATE recents
            ...
            Respond with just one line, nothing else. No explanation.
        """.trimIndent()
    ...
    fun parseGeminiResponse(geminiOutput: String): ParsedCommand {
        ...
        return when {
            ...
            upper.startsWith("SELFIE FRONT") || upper == "SELFIE" ->
                ParsedCommand.TakeSelfie(front = true)
            upper.startsWith("SELFIE BACK") ->
                ParsedCommand.TakeSelfie(front = false)
